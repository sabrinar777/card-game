...
function shuffle(deck)
{
  ...
  while (currentIndex != 0)
  {
    ...
    currentIndex--;
    // The values in shuffledDeck is swapped
...
{
  console.log(`Card1 Rank: ${card1.rank} Card1 Suit: ${card1.suit}`);
  let input = getInput("Do you think the next card will be higher (h) or lower (l)?")
  let compare_value
  if (input == "h")
  {
    compare_value = compare(card1,card2);
  ...
  {
    console.log("You must enter h or l. You get no points for this round.");
    return false;
  }
}
...
function playGame()
{
  ...
  {
    let nextCard = deck.pop() // Remove last object in deck array
    let outcome = guess(currentCard, nextCard);
    if (outcome == true) // Guess was correct
    {
      score++;
    ...
    {
      console.log("Your guess was not right! You get no points.");
    }
    currentCard = nextCard;
  }

  // Ternary statement to to see if the deck of cards is zero. If zero, then user lost. If not zero, then user won
  let results = (deck.length == 0) ? "You are out of cards and you lost" : "You won!";
  console.log(results);
}
